...
# split coordinate into x and y latitude, longitude
data["coordinates"] = data["coordinates"].str.replace("(", "")
data["coordinates"] = data["coordinates"].str.replace(")", "")

# confirm the changes
print(data["coordinates"].head())

# split the x, y in an array
coordinate_x_y = data["coordinates"].str.split(",", expand=True)
print(data["coordinates"].head())
# save the coordinate in separate variable
coordinate_x = coordinate_x_y[0]
coordinate_y = coordinate_x_y[1]
# add the two new variables to the dataframe
data["coordinate_x"] = coordinate_x
data["coordinate_y"] = coordinate_y
print(data.head())

# change the type of the variables
data["coordinate_x"] = data["coordinate_x"].astype(float)
data["coordinate_y"] = data["coordinate_y"].astype(float)
print(data.dtypes)

# delete the coordinates old column after splitting
